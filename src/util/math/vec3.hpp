#pragma once

#include "vec.hpp"

#include "nlohmann/json.hpp"

template <typename T>
struct vec<3, T>  {
	using value_type = T;
	using reference = value_type &;
	using const_reference = const value_type &;

	value_type x = value_type(0);
	value_type y = value_type(0);
	value_type z = value_type(0);

	constexpr vec() = default;
	constexpr vec(value_type x, value_type y, value_type z) : x{x}, y{y}, z{z} {}
	constexpr vec(value_type values[3]) : x{values[0]}, y{values[1]}, z{values[2]} {}

	constexpr reference operator[](size_t i) & {
		return this->*fields[i];
	}

	constexpr const_reference operator[](size_t i) const & {
		return this->*fields[i];
	}

	constexpr value_type operator[](size_t i) const && {
		return this->*fields[i];
	}

	friend void to_json(nlohmann::json& value, const vec& __self) {
        value = {
        	__self.x,
        	__self.y,
        	__self.z
		};
    }

    friend void from_json(const nlohmann::json& value, vec& __self) {
        value.at(0).get_to(__self.x);
        value.at(1).get_to(__self.y);
        value.at(2).get_to(__self.z);
    }

private:
	inline static constexpr float vec::* const fields[3] {
		&vec::x,
		&vec::y,
		&vec::z
	};

public:
	inline static const vec Zero{0, 0, 0};
	inline static const vec One{1, 1, 1};
	inline static const vec Right{1, 0, 0};
	inline static const vec Left{-1, 0, 0};
	inline static const vec Forward{0, 0, 1};
	inline static const vec Back{0, 0, -1};
	inline static const vec Up{0, 1, 0};
	inline static const vec Down{0, -1, 0};
};

using Vector3 = vec<3, float>;
using Vector3i = vec<3, int>;
