...
struct Input {
	void update() {
		...
		memcpy(_keysDownPrev, _keysDown, sizeof(_keysDown));
	...
	[[nodiscard]] std::pair<float, float> mouseDelta() const {
		return {_delta[0], _delta[1]};
	}

	[[nodiscard]] std::pair<float, float> mousePosition() const {
		return {_mousePosition[0], _mousePosition[1]};
	}
...
private:
	...
	bool _mouseDown[5]{};
	bool _mouseDownPrev[5]{};
	bool _keysDown[512]{};
	bool _keysDownPrev[512]{};
	float _mousePosition[2]{};
	float _delta[2]{};
};
